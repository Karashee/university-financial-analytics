...
def compute_variance_report(db: Session, dept_id_filter: Optional[str] = None) -> list[dict]:
    """
    Load expenditure_transactions, optionally filter by department, and
    compute variance statistics per (department_id, fiscal_year, fiscal_month).
...
def compute_growth_rate_summary(db: Session, dept_id_filter: Optional[str] = None) -> list[dict]:
    """
    Load expenditure_transactions, optionally filter by department, and
